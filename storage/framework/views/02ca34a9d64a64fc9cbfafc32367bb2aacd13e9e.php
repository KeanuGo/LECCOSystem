...
<div class="container">
	<div class="row">
	<?php echo Form::open(['url' => route('loans.store'), 'id'=>'add_loans_form']);; ?>

	<!--<form action="<?php echo e(route('loans.store')); ?>" id="add_loans_form" method="POST">-->
		<input type="hidden" value="<?php echo e(csrf_token()); ?>" form="add_loans_form">
		<table>
		<tr><td colspan =2><h3>Loan</h3></td></tr>
		<?php $__currentLoopData = $loans_values; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
			<tr>
			<td><?php echo e(ucwords(str_replace('_', ' ', $k))); ?></td>
			<td><?php echo e(Form::text("loans_values[$k]", json_decode(json_encode($v),true), ['readonly' => ''])); ?></td>
			</tr>
		<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
		<tr><td colspan=2><h3>Payrolls</h3></td></tr>
		<?php $__currentLoopData = $payrolls_values; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
			<tr>
			<td>
			<?php if($k == 'others'): ?>
				<?php echo e(ucwords(str_replace('_', ' ', $k)) .' : '. (json_encode($v))); ?>

			<?php else: ?>
				<?php echo e(ucwords(str_replace('_', ' ', $k))); ?>

			<?php endif; ?>
			</td>
			<td>
			<?php echo e(Form::hidden("payrolls_values[$k]", json_encode($v), ['readonly' => '', 'payroll_checkbox' => ''])); ?>
...
<script>
	...
	function generatePaymentApplied(){
		...
		//if(table){
			...
			var pded=[
				<?php $__currentLoopData = $payrolls_values; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
					<?php if($k == 'others'): ?>
						<?php $__currentLoopData = $v; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							'<?php echo e($k); ?>',
						<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
						<?php continue; ?>;
					<?php endif; ?>
	...
	function monthlyamortization1(principal, years, apr, count, pded)
	{
		...
		document.write("Monthly payment: " + numberWithCommas(roundtopennies(monthly(principal, years, apr)/count)));
		document.write("</th>");
		document.write("</tr>");
