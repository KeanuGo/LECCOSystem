    ...
    <div class="row">
        <div class="col-md-9 col-md-offset-2">
            <div class="panel panel-default">
				...
				{!! Form::open(['url' => route('check_voucher.update', ['id'=> $check_voucher['cv_no']]),'onsubmit' => 'return checkCVEntries()', 'enctype'=> 'multipart/form-data']); !!}
                <div class="panel-body">
				<table class='table table-striped'>
                        {{ csrf_field() }}
						@foreach($columns as $column)
						<tr>
							@if($column->COLUMN_NAME == 'cv_no')
								<td colspan = "2">
									@if ($message = Session::get('fail'))
										<div class="has-error">
									@endif
									{{ Form::label($column->COLUMN_NAME, 'CV No', array('class' => 'col-md-4 control-label')) }}
									<div class="col-md-6">
										{{ Form::number($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control')) }}
									</div>
									@continue;
									@if ($message = Session::get('fail'))
										</div>
							...
							@elseif(strpos($column->COLUMN_NAME, 'created_at') !== false or strpos($column->COLUMN_NAME, 'updated_at') !== false)
								@continue;
							@endif
								...
								<div class="col-md-6">
									...
									@elseif(strpos($column->TYPE_NAME, 'date') !== false)
										@if($column->NULLABLE == 1)
											{{ Form::date($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control')) }}
										@else
											{{ Form::date($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control', 'required' => '')) }}
										@endif
									...
									@elseif(strpos($column->TYPE_NAME, 'money') !== false or strpos($column->TYPE_NAME, 'decimal') !== false)
										@if($column->NULLABLE == 1)
											{{ Form::number($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control', 'step' => '0.01')) }}
										@else
											{{ Form::number($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control', 'step' => '0.01', 'required' => '')) }}
										@endif
									@elseif(strpos($column->TYPE_NAME, 'real') !== false)
										@if($column->NULLABLE == 1)
											{{ Form::number($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control', 'step' => 'any')) }}
										@else
											{{ Form::number($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control', 'step' => 'any', 'required' => '')) }}
										@endif
									...
									@else
										@if($column->NULLABLE == 1)
											{{ Form::text($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control' )) }}
										@else
											{{ Form::text($column->COLUMN_NAME, $check_voucher[$column->COLUMN_NAME], array('class' => 'form-control', 'required' => '')) }}
										@endif
									@endif
								</div>
								</td>
...
<script>
	...
	@for($i = 0; $i < count($cv_entries); $i++)
		...
		var code_field = document.getElementById('{{ $i }}');
		var debit_field = document.getElementById('cv_entries[{{$i}}][debit]');
		var credit_field = document.getElementById('cv_entries[{{$i}}][credit]');
		if(code_field && debit_field && credit_field){
			code_field.value = '{{ $cv_entries[$i]['account_code'] }}';
			code_field.setAttribute('value', '{{ $cv_entries[$i]['account_code'] }}');
			checkcoa('{{$i}}');
			debit_field.value = {{ $cv_entries[$i]['debit'] }};
			credit_field.value = {{ $cv_entries[$i]['credit'] }};
			debit_field.setAttribute('value', {{ $cv_entries[$i]['debit'] }});
			credit_field.setAttribute('value', {{ $cv_entries[$i]['credit'] }});
		}
	@endfor
	checkCVEntries();
</script>
