...
<div class="container">
	<table class="table table-striped">
		@foreach($check_voucher as $k => $v)
			@if(strpos($k, 'updated_at') !== false or strpos($k, 'avatar') !== false)
				@continue;
			@endif
			<tr>
				@if($k == 'name')
					<th>CV NO:</th>
				@elseif(strpos($k, 'attachment') !== false)
					<th>{{ ucwords(str_replace('_', ' ', $k)) }}</th>
					<td>
					{{ $v }}
					@if($v)
						<a class="link-tag delete-a" href="{{route('check_voucher.remove_attachment', ['id' => $check_voucher['cv_no']])}}">Remove Attachment</a>
					@endif
					</td>
					@continue;
				@else
	...
	<table class="table table-striped">
	<tr>
	@foreach($cv_entries[0] as $k => $v)
		@if(strpos($k, 'created_at') !== false or strpos($k, 'updated_at') !== false or strpos($k, 'avatar') !== false or strpos($k, 'id') !== false)
			@continue;
		@endif
			<th>{{ ucwords(str_replace('_', ' ', $k)) }}</th>
	@endforeach
	</tr>
	@for($i = 0 ; $i <  count($cv_entries) ; $i++)
		<tr>
		@foreach($cv_entries[$i] as $k => $v)
			@if(strpos($k, 'created_at') !== false or strpos($k, 'updated_at') !== false or strpos($k, 'avatar') !== false or strpos($k, 'id') !== false)
				@continue;
			@endif
				<td>{{ $v }}</td>
	...
	<hr>
	<div>
		<a href="{{route('check_voucher.edit', ['id' => $check_voucher['cv_no']])}}" class="link-tag edit-a">Edit</a>
		<a href="{{route('check_voucher.delete', ['id' => $check_voucher['cv_no']])}}" class="link-tag delete-a">Delete</a>
	...
	function pop_up() {
		...
		var CV_FORM = ''+
					...
					@foreach($signatories as $signatory)
						'<option value="{{$signatory['name']}}">{{$signatory['designation']}}</>'+
					@endforeach
					'</datalist>'+
					...
					'                    <label>'+ {{$check_voucher['cv_no']}}+'</label></td>'+
					'                <td style="text-align:right"><label>DATE:     </label>'+
					'                    <label>{{\Carbon\Carbon::parse($check_voucher['created_at'])->format('d-F-Y')}}</label></td>'+
					'            </tr>'+
					'            <tr>'+
					'                <td><label>PAYEE:     </label> '+
					'                    <label>{{$check_voucher['payee']}}</label></td>'+
					'            </tr>'+
					'        </table>'+
					'        '+
					'    </div>'+
					...
					'        <div class="desc-div">'+
					'            <br><label id="description">{{$check_voucher['description']}}</label><br><br>'+
					'        </div>'+
					'    </div>'+
					...
					'                    <td><i>{{$cv_entries[$i]['account_title']}}</i>'+
										  @if((strpos(strtolower($cv_entries[$i]['account_title']), 'cash') !== false) and (strpos(strtolower($cv_entries[$i]['account_title']), 'bank') !== false))
					'						  <script>'+
					'							var to_word = "{{ abs($cv_entries[$i]["debit"] - $cv_entries[$i]["credit"]) }}";'+
					'							var account_number = "{{ $cv_entries[$i]['account_title'] }}";'+
					...
					'						  <\/script>'+
										  @endif
					'					 </td>'+
					'                    <td> {{$cv_entries[$i]['account_code']}}'+
					'                    <td> {{$cv_entries[$i]['debit']==0?'':$cv_entries[$i]['debit']}} </td>'+
					'                    <td> {{$cv_entries[$i]['credit']==0?'':$cv_entries[$i]['credit']}} </td>'+
					'                </tr>'+
					'            @endfor'+
					'        </table>'+
					...
					'                    <label style="font-size:12px;">{{$check_voucher['check_no']}}</label>'+
					'                </td>'+
					'            </tr>'+
					'            <tr>'+
					'                <td>'+
					'                    <label style="font-size:12px;">Check Date:     </label>'+
					'                    <label style="font-size:12px;">{{\Carbon\Carbon::parse($check_voucher['created_at'])->format('d-F-Y')}}</label>'+
					'                </td>'+
					'            </tr>'+
					'        </table>'+
					'    </div><br>'+
					...
					'                <td>Date Disbursed:<br><input class="signature" value="{{$check_voucher['date_disbursed']}}"><br> </td>'+
					'            </tr>'+
					'        </table>'+
					'    </div>'+
					'    <br>'+
					...
					'                <td><label>{{$check_voucher['check_no']}}</label></td>'+
					'            </tr>'+
					'        </table><br>'+
					'        <div class="date-div">'+
					...
					'                    <td class="table-cell" style="width:200px;"><label>{{$check_voucher['payee']}}</label>'+
					'                    </td>'+
					...
					'</div>'+
					@else
					'<img style="display:block;height:260px;width:100%;" src="/storage/attachments/{{ $check_voucher['attachment'] }}" />'+
					@endif
					'<style>'+
					'    div.accounts-th-div {'+
					...
					'var signatories = {};'+
					@foreach($signatories as $signatory)
					'	signatories["{{$signatory["name"]}}"] = "{{$signatory["designation"]}}";'+
					@endforeach
